...
namespace NebulaCompatibilityAssist.Patches
{
    public static class AssemblerVerticalConstruction
    {
        ...
        {
            // Prevent exception
            if (assemblerId >= assemblerPool.Length || assemblerPool[assemblerId].served == null) return false;
            if (assemblerNextId >= assemblerPool.Length || assemblerPool[assemblerNextId].timeSpend == 0) return false;
